// Helper function from https://github.com/near-examples/near-multichain/blob/main/src/services/kdf.js
...
function sha256StringToScalarLittleEndian(hashString: string) {
  const matchResult = hashString.match(/../g) || [];
  const littleEndianString = matchResult.reverse().join("");
...
export async function deriveChildPublicKey(
  ...
  const scalar = sha256StringToScalarLittleEndian(hash);
...
export async function uncompressedHexPointToBtcAddress(publicKeyHex: string, network: Network) {
  ...
  const networkByte = Buffer.from([network.pubKeyHash]);
